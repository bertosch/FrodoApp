 ...
 */
public abstract class BotService extends IntentService {
    ...
    private final class StartBotThread extends Thread {
        ...
        @Override
        public void run() {
            Log.d("BotService", "StartBotThread run()");

            Intent loadingIntent =
                    new Intent(IntentActions.BROADCAST_ACTION_BOT_STATUS)
                            // Answer into Intent
                            .putExtra(IntentActions.EXTRA_BOT_STATUS, IntentActions.STATUS_BOT_LOADING);
            // Broadcast of Intent to Receiver in-app
            LocalBroadcastManager.getInstance(BotService.this).sendBroadcast(loadingIntent);
            ...
            // Broadcast of Intent to Receiver in-app
            LocalBroadcastManager.getInstance(BotService.this).sendBroadcast(loadedIntent);
        }
    }
}
